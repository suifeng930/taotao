...
<script type="text/javascript">
	var REGISTER={
		...
		beforeSubmit:function() {
				//检查用户是否已经被占用
				$.ajax({
	            	url : REGISTER.param.surl + "/user/check/"+escape($("#regName").val())+"/1?r=" + Math.random(), //Math.random()的作用是每次提交请求都是新的请求，避免浏览器缓存的影响
		...
		doSubmit:function() {
			...
			$.post("/user/register",$("#personRegForm").serialize(), function(data){//$("#personRegForm").serialize()，它的作用是将表单进行序列化，转换成key-value形式的字符串。
				if(data.status == 200){
					alert('用户注册成功，请登录！');
